...
class DiscoverHelper {
  ...
  static Future<ClientConfig?> discoverFromAutoConfigSubdomain(
    ...
    var response = await _HttpHelper.httpGet(url, connectionTimeout: _timeout);
    if (_isInvalidAutoConfigResponse(response)) {
      ...
      response = await _HttpHelper.httpGet(url, connectionTimeout: _timeout);
      if (_isInvalidAutoConfigResponse(response)) {
        return null;
      }
    }
  ...
  static Future<ClientConfig?> discoverFromIspDb(String? domain,
    ...
    final response =
        await _HttpHelper.httpGet(url, connectionTimeout: _timeout);
